...
COL={"integrated chain":"#2a78d6","unbranded independent":"#1baf7a","all":"#6b6b6b","bench":"#eb6834"}
...
def panel(ax, col, fn, ylabel, title, pct=False, bench_col=None):
    for j,t in enumerate(["integrated chain","unbranded independent","all"]):
        ...
        ax.errorbar(np.arange(4)+(j-1)*0.06, y, yerr=[y-lo,hi-y], fmt=MK[t]+"-", color=COL[t], lw=1.8, ms=6, capsize=2.5, label=LAB[t])
    if bench_col:
        bb=bench.copy(); bb["poids"]=1.0; v=series(bb,bench_col,fn); y,lo,hi=v[:,0],v[:,1],v[:,2]
        if pct: y,lo,hi=y*100,lo*100,hi*100
        ax.errorbar(np.arange(4)+0.12, y, yerr=[y-lo,hi-y], fmt=MK["bench"]+"--", color=COL["bench"], lw=1.4, ms=5, capsize=2.5, label=LAB["bench"])
    ax.set_xticks(range(4)); ax.set_xticklabels(xl); ax.set_ylabel(ylabel); ax.set_title(title, fontsize=10.5, loc="left"); ax.grid(axis="y", color="#e5e5e5", lw=.8); ax.spines[["top","right"]].set_visible(False)
    ax.set_xlabel("Competing hearing-aid centres within 10 km")
bench["rated"]=(bench["n_reviews"]>0).astype(float)
fig,axes=plt.subplots(1,2,figsize=(11,4.3))
panel(axes[0],"n_reviews",lambda x,w: wq(x,w,.5),"Median number of Google reviews","A. Review volume", bench_col="n_reviews")
panel(axes[1],"rated",wmean,"Sites with at least one review (%)","B. Share of sites with at least one review", pct=True, bench_col="rated")
axes[0].legend(frameon=False, fontsize=8.5); plt.tight_layout(); plt.savefig("outputs/v2/fig1_activity.png", dpi=220); plt.close()
...
ax=axes[0]
for j,(df,lab,c,m) in enumerate([(rated,"All rated centres","#6b6b6b","s"),(rated[rated["n_reviews"]>=10],"Centres with ≥10 reviews","#2a78d6","o"),(br,"Hairdressers, same communes","#eb6834","D")]):
    v=series(df,"rating_v" if "rating_v" in df else "rating",wmean); ax.errorbar(np.arange(4)+(j-1)*0.06, v[:,0], yerr=[v[:,0]-v[:,1],v[:,2]-v[:,0]], fmt=m+"-", color=c, lw=1.8, ms=6, capsize=2.5, label=lab)
ax.set_ylim(4.5,5.0); ax.set_xticks(range(4)); ax.set_xticklabels(xl); ax.set_ylabel("Mean Google rating"); ax.set_title("A. Rating level", fontsize=10.5, loc="left"); ax.grid(axis="y", color="#e5e5e5", lw=.8); ax.spines[["top","right"]].set_visible(False); ax.legend(frameon=False, fontsize=8.5); ax.set_xlabel("Competing hearing-aid centres within 10 km")
ax=axes[1]
for j,(df,lab,c,m,col) in enumerate([(top,"Hearing-aid centres","#2a78d6","o","rating_v"),(btop,"Hairdressers, same communes","#eb6834","D","rating")]):
    v=series(df,col,wsd); ax.errorbar(np.arange(4)+(j-0.5)*0.06, v[:,0], yerr=[v[:,0]-v[:,1],v[:,2]-v[:,0]], fmt=m+"-", color=c, lw=1.8, ms=6, capsize=2.5, label=lab)
ax.set_xticks(range(4)); ax.set_xticklabels(xl); ax.set_ylabel("S.D. of ratings across sites"); ax.set_title("B. Dispersion, top quartile of reviews within band", fontsize=10.5, loc="left"); ax.grid(axis="y", color="#e5e5e5", lw=.8); ax.spines[["top","right"]].set_visible(False); ax.legend(frameon=False, fontsize=8.5); ax.set_xlabel("Competing hearing-aid centres within 10 km")
ax=axes[2]
for j,(df,lab,c,m,col) in enumerate([(top,"Hearing-aid centres","#2a78d6","o","rating_v"),(btop,"Hairdressers, same communes","#eb6834","D","rating")]):
    df=df.copy(); df["b45"]=(df[col]<4.5).astype(float); v=series(df,"b45",wmean)*100; ax.errorbar(np.arange(4)+(j-0.5)*0.06, v[:,0], yerr=[v[:,0]-v[:,1],v[:,2]-v[:,0]], fmt=m+"-", color=c, lw=1.8, ms=6, capsize=2.5, label=lab)
ax.set_xticks(range(4)); ax.set_xticklabels(xl); ax.set_ylabel("Sites rated below 4.5 (%)"); ax.set_title("C. Low tail, top quartile of reviews within band", fontsize=10.5, loc="left"); ax.grid(axis="y", color="#e5e5e5", lw=.8); ax.spines[["top","right"]].set_visible(False); ax.set_xlabel("Competing hearing-aid centres within 10 km")
plt.tight_layout(); plt.savefig("outputs/v2/fig2_content.png", dpi=220); plt.close()
...
fig,ax=plt.subplots(figsize=(7.2,7.2)); cm=met(c); ax.scatter(cm["lon"],cm["lat"],s=0.15,color="#d9d9d9",rasterized=True)
cols={"10+":"#c9d6ea","3-9":"#86b6ef","1-2":"#eb6834","0":"#b30000"}; sizes={"10+":3,"3-9":5,"1-2":11,"0":16}
...
ax.set_aspect(1/np.cos(np.radians(46.5))); ax.axis("off"); ax.legend(frameon=False, fontsize=8.5, loc="lower left", markerscale=2)
ax.set_title("Hearing-aid centres in mainland France by local competition, August 2026", fontsize=10.5, loc="left"); plt.tight_layout(); plt.savefig("outputs/v2/fig0_map.png", dpi=220); plt.close()
